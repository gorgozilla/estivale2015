...
class EstivoleModelsWishlist extends EstivoleModelsDefault
{
  ...
  */
  protected function _buildQuery()
  {
    ...
    $query->from('#__estivole_books as b');

    $query->select('w.waitlist_id, w.user_id as borrower_id');
    $query->leftjoin('#__estivole_waitlists as w on w.book_id = b.book_id');

    $query->leftjoin("#__estivole_wishlists as wish on wish.book_id = b.book_id");
  ...
  function store()
  {
    ...
    $query = $db->getQuery(TRUE);
    $query->select('COUNT(*)');
    $query->from('#__estivole_wishlists');
